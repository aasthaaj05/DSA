...
void insert(int key, int value)
{
    ...
    while (array[i].flag == 1) 
    {
        if (array[i].data->key == key) 
        {
            ...
            free(new_item);  // free allocated memory as it's not needed
            return;
        }
...
void remove_element(int key)
{
    ...
    while (array[i].flag != 0) 
    {
        if (array[i].flag == 1  &&  array[i].data->key == key ) 
        {
            ...
            array[i].flag =  2;
            free(array[i].data);  // free the memory allocated for the data
            array[i].data = NULL;
            size--;
...
int main()
{
    ...
    free(array);  // free allocated memory for the hash table
    //getch();
    return 0;
}
